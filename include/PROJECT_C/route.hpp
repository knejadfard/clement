...
public:
    ...
    route(boost::beast::http::verb verb, const path& p)
        : verb_{verb}, path_{p}
    {
    }

    route(route const& rhs)
        : verb_{rhs.verb()}, path_{rhs.get_path()}
    {
    }

    route&
    operator=(route const& rhs)
    {
        verb_ = rhs.verb_;
        path_ = rhs.path_;
        return *this;
    ...
    verb_string() const
    {
        return boost::beast::http::to_string(verb_).to_string();
    }
...
std::ostream&
operator<<(std::ostream& out, const route& r)
{
    out << std::string(r);
    return out;
}
...
bool
operator!=(route const& lhs, route const& rhs)
{
    return !(lhs == rhs);
}
