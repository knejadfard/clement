...
class request_parser
{
    ...
    template <class Body>
    void read_fields(boost::beast::http::request<Body>&);

    // TODO this can simply read from headers_, since we have already parsed them
    template <class Body>
    void parse_target(boost::beast::http::request<Body>&);

    // TODO this can simply read from headers_, since we have already parsed them
    template <class Body>
    void parse_content_type(boost::beast::http::request<Body>&);
...
public:
    ...
    request_parser(boost::asio::ip::tcp::socket& socket) {
        parse(socket);
    }

    void parse(boost::asio::ip::tcp::socket&);

    void header(boost::beast::http::field const& name,
            std::string const& value) {
        headers_[name] = value;
    }

    std::string header(boost::beast::http::field const& name) const {
        std::map<boost::beast::http::field, std::string>::const_iterator it = headers_.find(name);
        if(it == headers_.end()) {
            return std::string();
        }
        return it->second;
    ...
    std::map<boost::beast::http::field, std::string> headers() const {
        return headers_;
    }
    ...
    std::string verb_string() const {
        return to_string(verb_).to_string();
    }
    ...
    route mapped_route() const {
        return mapped_route_;
    }

};

void
request_parser::parse(boost::asio::ip::tcp::socket& socket)
{
    ...
    boost::beast::http::request<boost::beast::http::dynamic_body> request = request_parser.get();
...
template <class Body>
void
request_parser::read_fields(boost::beast::http::request<Body>& request_header)
{
    for(typename boost::beast::http::request<Body>::const_iterator it =
            request_header.begin(); it != request_header.end(); ++it) {
...
template <class Body>
void
request_parser::parse_target(boost::beast::http::request<Body>& request_header)
{
    ...
    if(qmark_index == std::string::npos) {
        ...
        target_ = PROJECT_C::utility::percent_decode(unparsed_target);
    }
    else {
        target_ = PROJECT_C::utility::percent_decode(unparsed_target.substr(0, qmark_index));
        if(qmark_index != unparsed_target.size() - 1) {
            ...
            while(!done) {
                ...
                if(equal_sign_index == std::string::npos) {
                    throw std::runtime_error("Error parsing target string: " +
                            unparsed_target);
                }
                std::string param_name = PROJECT_C::utility::percent_decode(
                        query_param.substr(0, equal_sign_index));
                std::string param_value = PROJECT_C::utility::percent_decode(
...
template <class Body>
void
request_parser::parse_content_type(boost::beast::http::request<Body>& request_header)
{
...
void request_parser::parse_path_params()
{
    ...
    if(requested_frags.size() != mapped_frags.size()) {
        throw std::runtime_error("the size of requested and mapped routes do not match: <" +
                std::string{requested_path} + ">, <" + std::string{mapped_path} + ">");
    }
