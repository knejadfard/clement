...
public:
    ...
    logger()
            : file_name_{"output.log"} {
        initialize();
    }

    logger(std::string const& file_name)
    ...
    void initialize() {
        if (!initialized_) {
	        ...
	        log::formatter fmt = log::expressions::stream
                    << log::expressions::format_date_time<
    ...
    void info(const std::string& message) {
        BOOST_LOG_SEV(logger_, log::trivial::info) << message;
    }

    void error(std::string const& message) {
        BOOST_LOG_SEV(logger_, log::trivial::error) << message;
    }

    void debug(std::string const& message) {
        BOOST_LOG_SEV(logger_, log::trivial::debug) << message;
    }
